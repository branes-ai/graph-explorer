...
Capture_frame_command::Capture_frame_command(erhe::commands::Commands& commands, Explorer_context& explorer_context)
...
{
    m_context.explorer_rendering->trigger_capture();
    return true;
}
...
Explorer_rendering::Explorer_rendering(
    ...
    : m_context              {explorer_context}
    , m_capture_frame_command{commands, explorer_context}
...
{
    const auto& preset      = explorer_settings.graphics.current_graphics_preset;
...
void Explorer_rendering::handle_graphics_settings_changed(Graphics_preset* graphics_preset)
{
...
auto Explorer_rendering::get_shadow_node_for_view(const Scene_view& scene_view) -> std::shared_ptr<Shadow_render_node>
{
    auto i = std::find_if(
...
auto Explorer_rendering::get_all_shadow_nodes() -> const std::vector<std::shared_ptr<Shadow_render_node>>&
{
    return m_all_shadow_render_nodes;
}

auto Explorer_rendering::get_pipeline_renderpass(
...
auto Explorer_rendering::make_renderpass(const std::string_view name) -> std::shared_ptr<Renderpass>
{
...
void Explorer_rendering::set_tool_scene_root(Scene_root* in_tool_scene_root)
{
    this->tool_scene_root = in_tool_scene_root;
}

void Explorer_rendering::trigger_capture()
{
    m_trigger_capture = true;
}

auto Explorer_rendering::width() const -> int
{
    return m_context.context_window->get_width();
}

auto Explorer_rendering::height() const -> int
{
    return m_context.context_window->get_height();
}

void Explorer_rendering::imgui()
{
    ERHE_PROFILE_FUNCTION();
...
void Explorer_rendering::begin_frame()
{
    ERHE_PROFILE_FUNCTION();
...
void Explorer_rendering::request_renderdoc_capture()
{
#if defined(ERHE_XR_LIBRARY_OPENXR)
...
void Explorer_rendering::end_frame()
{
    ERHE_PROFILE_FUNCTION();
...
void Explorer_rendering::add(Renderable* renderable)
{
    ...
    if (i != m_renderables.end()) {
        log_render->error("Explorer_rendering::add(Renderable*): renderable is already registered");
        return;
    }
#endif
...
void Explorer_rendering::remove(Renderable* renderable)
{
    const std::lock_guard<ERHE_PROFILE_LOCKABLE_BASE(std::mutex)> lock{m_renderables_mutex};
    ...
    if (i == m_renderables.end()) {
        log_render->error("Explorer_rendering::remove(Renderable*): renderable is not registered");
        return;
    }
    m_renderables.erase(i);
}

void Explorer_rendering::render_viewport_main(const Render_context& context)
{
    ERHE_PROFILE_FUNCTION();
...
void Explorer_rendering::render_viewport_renderables(const Render_context& context)
{
    ERHE_PROFILE_FUNCTION();
...
void Explorer_rendering::render_composer(const Render_context& context)
{
...
void Explorer_rendering::render_id(const Render_context& context)
{
    ERHE_PROFILE_FUNCTION();
