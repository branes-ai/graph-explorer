...
Hud_drag_command::Hud_drag_command(erhe::commands::Commands& commands, Explorer_context& context)
...
Toggle_hud_visibility_command::Toggle_hud_visibility_command(erhe::commands::Commands& commands, Explorer_context& context)
...
#if defined(ERHE_XR_LIBRARY_OPENXR)
    , m_drag_command                      {commands, explorer_context}
...
#endif
{
    ...
    m_rendertarget_mesh->enable_flag_bits(
        ...
        (explorer_context.developer_mode ? erhe::Item_flags::show_in_ui : 0)
    );
    ...
    m_rendertarget_imgui_viewport = std::make_shared<explorer::Rendertarget_imgui_host>(
        imgui_renderer,
        rendergraph,
        explorer_context,
    ...
    m_rendertarget_imgui_viewport->set_begin_callback(
        [&explorer_windows](erhe::imgui::Imgui_host& imgui_host) {
            explorer_windows.viewport_menu(imgui_host);
        }
    );
...
void Hud::on_message(Explorer_message& message)
{
