...
Debug_view_window::Debug_view_window(
    ...
    , m_node                   {rendergraph}
{
    if (explorer_context.OpenXR || !explorer_context.developer_mode) {
        hide_window();
        hidden();
        return;
    }
    ...
    const auto& nodes = explorer_rendering.get_all_shadow_nodes();
    if (!nodes.empty()) {
...
void Debug_view_window::imgui()
{
    ...
    const auto& nodes = m_context.explorer_rendering->get_all_shadow_nodes();
    if (!nodes.empty()) {
