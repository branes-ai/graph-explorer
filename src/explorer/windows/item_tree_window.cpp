...
auto Item_tree::item_icon_and_text(const std::shared_ptr<erhe::Item_base>& item, const bool update, bool force_expand) -> Tree_node_state
{
    ...
    const auto& node = std::dynamic_pointer_cast<erhe::scene::Node>(item);
    if (!m_context.explorer_settings->node_tree_expand_attachments && node) {
...
void Item_tree::imgui_item_node(const std::shared_ptr<erhe::Item_base>& item)
{
    ...
    if (tree_node_state.is_open) {
        if (m_context.explorer_settings->node_tree_expand_attachments) {
            const auto& node = std::dynamic_pointer_cast<erhe::scene::Node>(item);
            if (node) {
...
void Item_tree::imgui_tree(float ui_scale)
{
...
#if 0 //// TODO
    ImGui::Checkbox("Expand Attachments", &m_context.explorer_settings->node_tree_expand_attachments);
    ImGui::Checkbox("Show All",           &m_context.explorer_settings->node_tree_show_all);
#endif
    ...
    if (ImGui::Button("Create Scene")) {
        ...
        m_context.explorer_scenes->register_scene_root(scene_root);
    }
#endif
    //// const auto& scene_roots = m_context.explorer_scenes->get_scene_roots();
    //// for (const auto& scene_root : scene_roots) {
