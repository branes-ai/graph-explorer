...
void Explorer_windows::renderdoc_capture()
{
    if (m_context.renderdoc) {
#if defined(ERHE_XR_LIBRARY_OPENXR)
        if (m_context.OpenXR && m_context.headset_view->is_active()) {
            m_context.explorer_rendering->request_renderdoc_capture();
        }
        else
#endif
        {
...
void Explorer_windows::builtin_imgui_window_menu()
{
...
void Explorer_windows::viewport_menu(erhe::imgui::Imgui_host& imgui_host)
{
