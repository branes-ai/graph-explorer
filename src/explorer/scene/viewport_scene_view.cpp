...
void Viewport_scene_view::execute_rendergraph_node()
{
    ...
    if (do_render && m_is_scene_view_hovered && m_context.id_renderer->enabled) {
        m_context.explorer_rendering->render_id(context);
    }
    ...
    m_context.explorer_rendering->render_viewport_main(context);

    m_context.line_renderer   ->open();
    m_context.tools           ->render_viewport_tools(context);
    m_context.explorer_rendering->render_viewport_renderables(context);
...
auto Viewport_scene_view::viewport_toolbar() -> bool
{
    ...
    ImGui::SetNextItemWidth(110.0f);
    const bool combo_used     = m_context.explorer_scenes->scene_combo("##Scene", scene_root_raw, false);
    if (ImGui::IsItemHovered()) {
        hovered = true;
    }
    if (combo_used) {
