...
Physics_window::Physics_window(erhe::imgui::Imgui_renderer& imgui_renderer, erhe::imgui::Imgui_windows& imgui_windows, Explorer_context& explorer_context)
...
{
    bool physics_enabled = m_context.explorer_settings->physics.dynamic_enable;
    ...
    if (pressed && m_context.explorer_settings->physics.static_enable) {
        m_context.explorer_settings->physics.dynamic_enable = !m_context.explorer_settings->physics.dynamic_enable;
    }
}
...
void Physics_window::imgui()
{
    ...
    if (!m_context.explorer_settings->physics.static_enable) {
        ImGui::BeginDisabled();
    }
    ImGui::Checkbox("Physics enabled", &m_context.explorer_settings->physics.dynamic_enable);
    ImGui::Checkbox("Debug draw", &m_context.explorer_settings->physics.debug_draw);
    if (!m_context.explorer_settings->physics.static_enable) {
        ImGui::EndDisabled();
        if (ImGui::IsItemHovered()) {
