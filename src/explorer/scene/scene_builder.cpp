    ...
    : m_context{explorer_context}
{
    ...
    setup_lights();
    make_brushes(explorer_settings, mesh_memory, executor);
    add_room    ();
}
...
)
{
    ...
    const int msaa_sample_count = explorer_settings.graphics.current_graphics_preset.msaa_sample_count;
...
void Scene_builder::make_platonic_solid_brushes(Explorer_settings& explorer_settings, Mesh_memory& mesh_memory)
{
    ERHE_PROFILE_FUNCTION();
    ...
    m_platonic_solids.push_back(make_brush(folder, explorer_settings, mesh_memory, dodecahedron));
    ...
    m_platonic_solids.push_back(make_brush(folder, explorer_settings, mesh_memory, icosahedron));
    ...
    m_platonic_solids.push_back(make_brush(folder, explorer_settings, mesh_memory, octahedron));
    ...
    m_platonic_solids.push_back(make_brush(folder, explorer_settings, mesh_memory, cuboctahedron));
    ...
    m_platonic_solids.push_back(make_brush(folder, explorer_settings, mesh_memory, tetrahedron));
...
void Scene_builder::make_sphere_brushes(Explorer_settings& explorer_settings, Mesh_memory& mesh_memory)
{
    ERHE_PROFILE_FUNCTION();
...
void Scene_builder::make_torus_brushes(Explorer_settings& explorer_settings, Mesh_memory& mesh_memory)
{
    ERHE_PROFILE_FUNCTION();
...
void Scene_builder::make_cylinder_brushes(Explorer_settings& explorer_settings, Mesh_memory& mesh_memory)
{
    ERHE_PROFILE_FUNCTION();
...
void Scene_builder::make_cone_brushes(Explorer_settings& explorer_settings, Mesh_memory& mesh_memory)
{
    ERHE_PROFILE_FUNCTION();
...
void Scene_builder::make_json_brushes(Explorer_settings& explorer_settings, Mesh_memory& mesh_memory, tf::Taskflow* tf, Json_library& library)
{
    ERHE_PROFILE_FUNCTION();
    ...
    for (const auto& key_name : library.names) {
        auto op = [this, &explorer_settings, &mesh_memory, &library, &key_name, &folder]() {
...
void Scene_builder::make_brushes(Explorer_settings& explorer_settings, Mesh_memory& mesh_memory, tf::Executor& executor)
{
    ERHE_PROFILE_FUNCTION();
    ...
    if (m_config.floor) {
        ...
        //    [this, &floor_box_shape, &explorer_settings, &mesh_memory]() {
        {
    ...
    if (executor.num_workers() > 1) {
        ...
        tf.emplace([this, &explorer_settings, &mesh_memory]() { make_platonic_solid_brushes(explorer_settings, mesh_memory); }).name("Platonic Solid Brushes");
        tf.emplace([this, &explorer_settings, &mesh_memory]() { make_sphere_brushes        (explorer_settings, mesh_memory); }).name("Sphere Brushes");
        tf.emplace([this, &explorer_settings, &mesh_memory]() { make_torus_brushes         (explorer_settings, mesh_memory); }).name("Torus Brushes");
        tf.emplace([this, &explorer_settings, &mesh_memory]() { make_cylinder_brushes      (explorer_settings, mesh_memory); }).name("Cylinder Brushes");
        tf.emplace([this, &explorer_settings, &mesh_memory]() { make_cone_brushes          (explorer_settings, mesh_memory); }).name("Cone Brushes");

        make_json_brushes(explorer_settings, mesh_memory, &tf, library);

        tf::Future<void> future = executor.run(tf);
        future.wait();
    } else {
        make_platonic_solid_brushes(explorer_settings, mesh_memory);
        make_sphere_brushes        (explorer_settings, mesh_memory);
        make_torus_brushes         (explorer_settings, mesh_memory);
        make_cylinder_brushes      (explorer_settings, mesh_memory);
        make_cone_brushes          (explorer_settings, mesh_memory);

        make_json_brushes(explorer_settings, mesh_memory, nullptr, library);
    }
