...
class Explorer : public erhe::window::Input_event_handler
{
public:
    void tick()
    {
        ...
        // log_input_frame->trace("----------------------- Explorer::tick() -----------------------");
        ...
        if (!m_explorer_context.OpenXR) { //if (!m_headset_view.is_active()) {
        ...
        m_explorer_scenes->before_physics_simulation_steps();
        ...
        m_time->for_each_fixed_step(
            [this, &input_events, &host_system_dt_s, &host_system_time_ns](const Time_context& time_context) {
                ...
                m_explorer_scenes->update_physics_simulation_fixed_step(time_context);
            }
        );
        m_explorer_scenes->after_physics_simulation_steps();
        ...
        //    - Explorer_scenes (updates physics)
        //    - Fly_camera_tool
        //    - Network_window 
        m_explorer_message_bus->update(); // Flushes queued messages
        ...
        m_explorer_rendering->begin_frame(); // tests renderdoc capture start

        // Execute rendergraph
        m_rendergraph->execute();

        m_imgui_renderer->next_frame();
        m_explorer_rendering->end_frame();
        if (!m_explorer_context.OpenXR) {
            gl::bind_framebuffer(gl::Framebuffer_target::framebuffer, 0);
            if (m_explorer_context.use_sleep) {
                ...
                const std::chrono::duration<float> sleep_margin{m_explorer_context.sleep_margin};
    ...
    [[nodiscard]] auto create_window() -> std::unique_ptr<erhe::window::Context_window>
    {
        ...
        headset_section.get("openxr",        m_explorer_context.OpenXR);
        headset_section.get("openxr_mirror", m_explorer_context.OpenXR_mirror);

        const auto& developer_section = erhe_ini.get_section("developer");
        developer_section.get("enable", m_explorer_context.developer_mode);

        const auto& renderdoc_section = erhe_ini.get_section("renderdoc");
        renderdoc_section.get("capture_support", m_explorer_context.renderdoc);
        if (m_explorer_context.renderdoc) {
            m_explorer_context.developer_mode = true;
        }

        const auto& window_section = erhe_ini.get_section("window");
        window_section.get("use_sleep", m_explorer_context.use_sleep);
        window_section.get("sleep_margin", m_explorer_context.sleep_margin);

        erhe::window::Window_configuration configuration{
            .use_depth         = m_explorer_context.OpenXR_mirror,
            .use_stencil       = m_explorer_context.OpenXR_mirror,
            .gl_major          = 4,
            .gl_minor          = 6,
            .width             = 1920,
            .height            = 1080,
    ...
    Explorer()
    {
        ...
        try {
            ...
            Icons icons;
            Icon_loader icon_loader{m_explorer_settings->icon_settings};
            ...
            m_explorer_settings->apply_limits(explorer_message_bus);
            ...
            if (m_explorer_context.OpenXR) {
                ...
                if (!m_headset->is_valid()) {
                    ...
                    m_explorer_context.OpenXR = false;
                    m_headset.reset();
                }
            }
#endif
            ...
            {
                erhe::graphics::Scoped_gl_context ctx{m_graphics_instance->context_provider};
                m_imgui_renderer = std::make_unique<erhe::imgui::Imgui_renderer>(*m_graphics_instance.get(), m_explorer_settings->imgui);
            })  .name("Imgui_renderer");
            ...
            auto icon_set_task = taskflow.emplace([this, &icons, &icon_loader]()
            {
                ...
                m_icon_set = std::make_unique<Icon_set>(
                    ...
                    *m_graphics_instance.get(),
                    m_explorer_settings->icon_settings,
                    icons,
                    icon_loader
                );
            })  .name("Icon_set")
            ...
            {
                erhe::graphics::Scoped_gl_context ctx{m_graphics_instance->context_provider};
                m_post_processing = std::make_unique<Post_processing>(*m_graphics_instance.get(), m_explorer_context);
            ...
            auto explorer_rendering_task = taskflow.emplace([this]()
            {
                erhe::graphics::Scoped_gl_context ctx{m_graphics_instance->context_provider};
                m_explorer_rendering = std::make_unique<Explorer_rendering>(
                    *m_commands.get(),
                    *m_graphics_instance.get(),
                    m_explorer_context,
            ...
            auto some_windows_task = taskflow.emplace([this]()
            {
                ...
                m_timeline_window        = std::make_unique<Timeline_window                 >(*m_imgui_renderer.get(), *m_imgui_windows.get(),  m_explorer_context);
                m_icon_browser           = std::make_unique<Icon_browser                    >(*m_imgui_renderer.get(), *m_imgui_windows.get());
            ...
            auto default_scene_task = taskflow.emplace([this]()
            {
                ...
                m_default_scene = std::make_shared<Scene_root>(
                    ...
                    *m_scene_message_bus.get(),
                    &m_explorer_context,
                    m_explorer_message_bus.get(),
                    m_explorer_scenes.get(),
                    content_library,
                    "Default Scene"
                );
                m_default_scene_browser = m_default_scene->make_browser_window(
                    *m_imgui_renderer.get(), *m_imgui_windows.get(), m_explorer_context, *m_explorer_settings.get()
                );
            })  .name("Default scene")
                .succeed(imgui_renderer_task, imgui_windows_task);
            ...
            auto scene_builder_task = taskflow.emplace([this]()
            {
                ...
                .succeed(default_scene_task, imgui_renderer_task, imgui_windows_task, rendergraph_task, explorer_rendering_task, mesh_memory_task, post_processing_task, tools_task);
            ...
            auto headset_task = taskflow.emplace([this]()
            {
                ...
                m_headset_view = std::make_unique<Headset_view>(
                    ...
                    *m_context_window.get(),
                    ...
                    m_explorer_context,
                    *m_explorer_rendering.get(),
                    *m_explorer_settings.get()
                );
#if defined(ERHE_XR_LIBRARY_OPENXR)
                if (m_explorer_context.OpenXR) {
                    m_hand_tracker = std::make_unique<Hand_tracker>(m_explorer_context, *m_explorer_rendering.get());
                }
#endif
            })  .name("Headset (init)")
                .succeed(imgui_renderer_task, imgui_windows_task, rendergraph_task, explorer_rendering_task);
            ...
            {
                erhe::graphics::Scoped_gl_context ctx{m_graphics_instance->context_provider};
                m_move_tool   = std::make_unique<Move_tool  >(m_explorer_context, *m_icon_set.get(), *m_tools.get());
                m_rotate_tool = std::make_unique<Rotate_tool>(m_explorer_context, *m_icon_set.get(), *m_tools.get());
                m_scale_tool  = std::make_unique<Scale_tool >(m_explorer_context, *m_icon_set.get(), *m_tools.get());
        ...
        fill_explorer_context();

        const auto& physics_section = erhe::configuration::get_ini_file_section("erhe.ini", "physics");
        physics_section.get("static_enable",  m_explorer_settings->physics.static_enable);
        physics_section.get("dynamic_enable", m_explorer_settings->physics.dynamic_enable);
        if (!m_explorer_settings->physics.static_enable) {
            m_explorer_settings->physics.dynamic_enable = false;
        }
        ...
        if (!m_explorer_context.OpenXR) {
            window_viewport->set_begin_callback(
                [this](erhe::imgui::Imgui_host& imgui_host) {
                    m_explorer_windows->viewport_menu(imgui_host);
                }
            );
        }
    ...
    void fill_explorer_context()
    {
        ERHE_PROFILE_FUNCTION();

        m_explorer_context.commands               = m_commands              .get();
        m_explorer_context.graphics_instance      = m_graphics_instance     .get();
        m_explorer_context.imgui_renderer         = m_imgui_renderer        .get();
        m_explorer_context.imgui_windows          = m_imgui_windows         .get();
#if defined(ERHE_PHYSICS_LIBRARY_JOLT)
        m_explorer_context.debug_renderer         = m_debug_renderer        .get();
#endif
        m_explorer_context.line_renderer          = m_line_renderer         .get();
        m_explorer_context.text_renderer          = m_text_renderer         .get();
        m_explorer_context.rendergraph            = m_rendergraph           .get();
        m_explorer_context.scene_message_bus      = m_scene_message_bus     .get();
        m_explorer_context.forward_renderer       = m_forward_renderer      .get();
        m_explorer_context.shadow_renderer        = m_shadow_renderer       .get();
        m_explorer_context.context_window         = m_context_window        .get();
        m_explorer_context.brdf_slice             = m_brdf_slice            .get();
        m_explorer_context.brush_tool             = m_brush_tool            .get();
        m_explorer_context.clipboard              = m_clipboard             .get();
        m_explorer_context.clipboard_window       = m_clipboard_window      .get();
        m_explorer_context.create                 = m_create                .get();
        m_explorer_context.explorer_message_bus   = m_explorer_message_bus  .get();
        m_explorer_context.explorer_rendering     = m_explorer_rendering    .get();
        m_explorer_context.explorer_scenes        = m_explorer_scenes       .get();
        ...
        m_explorer_context.icon_browser                   = m_icon_browser          .get();
        ...
        m_explorer_context.timeline_window                = m_timeline_window       .get();
    ...
    void run()
    {
        ...
        m_run_started = true;
        float wait_time = m_explorer_context.use_sleep ? m_explorer_context.sleep_margin : 0.0f;
    ...
    std::unique_ptr<Explorer_rendering              >        m_explorer_rendering;
...
void run_explorer()
{
    ...
    {
        ERHE_PROFILE_SCOPE("Construct and run Explorer");
