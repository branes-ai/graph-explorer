...
Scene_root::Scene_root(
    ...
    Explorer_scenes*                        explorer_scenes,
    ...
    : m_content_library{content_library}
{
    ...
    if (explorer_scenes != nullptr) {
        register_to_explorer_scenes(*explorer_scenes);
    }

    if ((imgui_renderer != nullptr) && (imgui_windows  != nullptr) && (explorer_context != nullptr)) {
    ...
    if (explorer_message_bus != nullptr) {
        explorer_message_bus->add_receiver(
            [this](Explorer_message& message) {
...
{
    if (m_is_registered) { // && (m_explorer_scenes != nullptr)) {
        unregister_from_explorer_scenes(*m_explorer_scenes);
    }
}
...
) -> std::shared_ptr<Item_tree_window>
{
    ...
    m_node_tree_window->set_item_filter(
        explorer_settings.node_tree_show_all
            ? erhe::Item_filter{
                .require_all_bits_set           = 0,
                .require_at_least_one_bit_set   = 0,
...
void Scene_root::register_to_explorer_scenes(Explorer_scenes& explorer_scenes)
{
    ERHE_VERIFY(m_is_registered == false);
    ERHE_VERIFY(m_explorer_scenes == nullptr);
    m_explorer_scenes = &explorer_scenes;
    explorer_scenes.register_scene_root(this);
    m_is_registered = true;
}

void Scene_root::unregister_from_explorer_scenes(Explorer_scenes& explorer_scenes)
{
    ERHE_VERIFY(m_is_registered == true);
    ERHE_VERIFY(m_explorer_scenes == &explorer_scenes);
    m_explorer_scenes = nullptr;
    explorer_scenes.unregister_scene_root(this);
