...
Open_new_viewport_scene_view_command::Open_new_viewport_scene_view_command(erhe::commands::Commands& commands, Explorer_context& explorer_context)
...
Scene_views::Scene_views(
    ...
    : m_context                             {explorer_context}
    , m_open_new_viewport_scene_view_command{commands, explorer_context}
{
...
void Scene_views::on_message(Explorer_message& message)
{
...
) -> std::shared_ptr<Viewport_scene_view>
{
    ...
    if (explorer_settings.graphics.current_graphics_preset.shadow_enable) {
        //// TODO: Share Shadow_render_node for each unique (scene, camera) pair
        const auto shadow_render_node = explorer_rendering.create_shadow_node_for_scene_view(
            graphics_instance,
            rendergraph,
            explorer_settings,
...
auto Scene_views::open_new_viewport_scene_view(const std::shared_ptr<Scene_root>& scene_root) -> std::shared_ptr<Viewport_scene_view>
{
    ...
    const int msaa_sample_count = m_context.explorer_settings->graphics.current_graphics_preset.msaa_sample_count;
    if (scene_root) {
