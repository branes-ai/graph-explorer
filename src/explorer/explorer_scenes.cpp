...
Explorer_scenes::Explorer_scenes(Explorer_context& explorer_context)
    : m_context{explorer_context}
{
}

void Explorer_scenes::register_scene_root(Scene_root* scene_root)
{
    ...
    if (i != m_scene_roots.end()) {
        log_scene->error("Scene '{}' is already in registered in Explorer_scenes", scene_root->get_name());
    } else {
        m_scene_roots.push_back(scene_root);
    }
}

void Explorer_scenes::unregister_scene_root(Scene_root* scene_root)
{
    const std::lock_guard<ERHE_PROFILE_LOCKABLE_BASE(std::mutex)> lock{m_mutex};

    const auto i = std::remove(m_scene_roots.begin(), m_scene_roots.end(), scene_root);
    if (i == m_scene_roots.end()) {
        log_scene->error("Scene '{}' not registered in Explorer_scenes", scene_root->get_name());
    } else {
        m_scene_roots.erase(i, m_scene_roots.end());
    }
}

void Explorer_scenes::imgui()
{
...
void Explorer_scenes::update_physics_simulation_fixed_step(const Time_context& time_context)
{
    ERHE_PROFILE_FUNCTION();

    if (
        !m_context.explorer_settings->physics.static_enable ||
        !m_context.explorer_settings->physics.dynamic_enable
    ) {
        return;
    }
...
void Explorer_scenes::before_physics_simulation_steps()
{
    ERHE_PROFILE_FUNCTION();

    if (!m_context.explorer_settings->physics.static_enable || !m_context.explorer_settings->physics.dynamic_enable) {
        return;
    }
...
void Explorer_scenes::update_node_transforms()
{
    ERHE_PROFILE_FUNCTION();
...
void Explorer_scenes::after_physics_simulation_steps()
{
    ERHE_PROFILE_FUNCTION();
...
auto Explorer_scenes::get_scene_roots() -> const std::vector<Scene_root*>&
{
    return m_scene_roots;
}

void Explorer_scenes::sanity_check()
{
#if !defined(NDEBUG)
    for (const auto& scene_root : m_scene_roots) {
...
auto Explorer_scenes::scene_combo(const char* label, Scene_root*& in_out_selected_entry, const bool empty_option) const -> bool
{
