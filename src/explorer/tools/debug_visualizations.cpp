...
{
    const auto shadow_render_node = m_context.explorer_rendering->get_shadow_node_for_view(context.render_context.scene_view);
...
void Debug_visualizations::spot_light_visualization(const Light_visualization_context& context)
{
    ...
    //auto* explorer_time = get<Explorer_time>();
    //const float time = static_cast<float>(explorer_time->time());
    //const float half_position = (explorer_time != nullptr)
    //    ? time - floor(time)
    //    : 0.5f;
...
#if defined(ERHE_PHYSICS_LIBRARY_JOLT)
    Explorer_context& explorer_context = context.explorer_context;
    if (explorer_context.explorer_settings->physics.debug_draw) {
        ...
        explorer_context.debug_renderer->SetCameraPos(camera_position_jolt);
