...
{
    Icons icons;
    Icon_loader icon_loader{m_context.explorer_settings->icon_settings};
    ...
    m_context.icon_set->load_icons(
        *m_context.graphics_instance,
        m_context.explorer_settings->icon_settings,
        icons,
        icon_loader
    );
}
...
void Settings_window::imgui()
{
    ...
    const float ui_scale = m_context.explorer_settings->get_ui_scale();
    ...
    add_entry("UI Font Size", [this](){
        auto& imgui = m_context.explorer_settings->imgui;
    ...
    add_entry("Small Icon Size", [this](){
        int* icon_size = &m_context.explorer_settings->icon_settings.small_icon_size;
    ...
    add_entry("Large Icon Size", [this](){
        int* icon_size = &m_context.explorer_settings->icon_settings.large_icon_size;
        const int old_icon_size = *icon_size;
        ImGui::DragInt("##", icon_size, 0.1f, 4, 512);
        if (ImGui::IsItemDeactivatedAfterEdit() && old_icon_size != *icon_size) {
            rasterize_icons();
    ...
    add_entry("Hotbar Icon Size", [this](){
        int* icon_size = &m_context.explorer_settings->icon_settings.hotbar_icon_size;
        const int old_icon_size = *icon_size;
        ImGui::DragInt("##", icon_size, 0.1f, 4, 512);
        if (ImGui::IsItemDeactivatedAfterEdit() && old_icon_size != *icon_size) {
            rasterize_icons();
    ...
    auto& graphics = m_context.explorer_settings->graphics;
    ...
    add_entry("", [this, button_size, &graphics_presets](){
        ...
        if (ImGui::Button("Use", button_size)) {
            m_context.explorer_settings->graphics.current_graphics_preset = graphics_presets.at(m_graphics_preset_index);
            m_context.explorer_message_bus->send_message(
                Explorer_message{
                    .update_flags    = Message_flag_bit::c_flag_bit_graphics_settings,
                    .graphics_preset = &m_context.explorer_settings->graphics.current_graphics_preset
                }
            );
        }
    });
    ...
    add_entry("", [this, button_size](){
        if (ImGui::Button("Load", button_size)) {
            m_context.explorer_settings->read();
        }
        ImGui::SameLine();
        if (ImGui::Button("Save", button_size)) {
            m_context.explorer_settings->write();
        }
    });
